...
const {logWho} = require('./getPatientRecordSpecificOrg3.js');
...
async function main() {
    try {
        ...
        const identity = await wallet.get(logWho.account+'Org3');

        // Extract the value between single quotes and store it in the variable 'who'
        const who = identity ? logWho.account+'Org3' : logWho.account+'Org3';
        if (!identity) {
        ...
        await contract.submitTransaction('createCar', currentTime, 'Org3MSP', who, 'Query Patient Records', currentTime, who,);
