...
//const { createLog } = require('./createAuditRecordGetPatientOrg1'); // Include create_log.js
...
async function main() {
  ...
  module.exports = { logWho };
        ...
        if (!identity) {
            ...
            const { createLog } = require('./createAuditRecordFailGetAuditOrg2.js'); // Include create_log.js
            return;
        }
        ...
        if (clientOrg !== 'Org2MSP') {
            throw new Error(`Access denied. Only Org2MSP organization is authorized to query the chaincode.`);
        }
        ...
        // Filter the results to include only entries that have Org2MSP
        const filteredResults = allResults.filter((record) => record.Record.who === user+'Org2');

        console.log(`Transaction has been evaluated, result is:`);
        console.log(filteredResults);

        const { createLog } = require('./createAuditRecordGetAuditSpecificOrg2.js'); // Include create_log.js
