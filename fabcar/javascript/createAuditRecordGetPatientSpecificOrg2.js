...
const {logWho} = require('./getPatientRecordSpecificOrg2.js');
...
async function main() {
    try {
        ...
        const identity = await wallet.get(logWho.account+'Org2');

        // Extract the value between single quotes and store it in the variable 'who'
        const who = identity ? logWho.account+'Org2' : logWho.account+'Org2';
        if (!identity) {
        ...
        await contract.submitTransaction('createCar', currentTime, 'Org2MSP', who, 'Query Patient Records', currentTime, who,);
