...
async function main() {
  ...
  module.exports = { logWho };
        ...
        if (!identity) {
            ...
            const { createLog } = require('./createAuditRecordFailGetPatientOrg3'); // Include create_log.js
            return;
        }
        ...
        const result = await contract.submitTransaction('queryAllCars');
        const allResults = JSON.parse(result.toString());

        // Filter the results to include only entries that have Org3MSP
        const filteredResults = allResults.filter((record) => record.Record.name === user+'Org3');

        console.log(`Transaction has been evaluated, result is:`);
        console.log(filteredResults);
        const { createLog } = require('./createAuditRecordGetPatientSpecificOrg3'); // Include create_log.js
